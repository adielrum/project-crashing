...
We find two surprising relationships in @fig:oat-T-max. First, total project cost is almost perfectly linearly related to the target deadline. This further corroborates the time--based model's assumption of linearity. However, second, this total cost is achieved through a non--linear crashing strategy. The Cobb--Douglas model provides a crashing solution that always meets every target deadline but is vastly different depending on the given deadline. For $T_max < 230$, we see that the model approximately linearly shrinks the completion day to account for rising costs and tighter deadlines. On the other hand, when $T_max > 230$, the optimal makespan has no obvious pattern, but stabilizing between 216 and 219 days. A possible explanation is that longer deadlines allow the influence of bonuses to become more potent, which frees the model from a strictly linear crashing approach. This proves the model's flexibility to provide crashing solutions for various project deadlines while maintaining predictable cost variations. 
