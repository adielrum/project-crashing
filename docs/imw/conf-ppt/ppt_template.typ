...
#let setup(
	...
	show ref: it => text(fill: priv-primary.get())[#it]
	show link: it => text(fill: priv-primary.get())[#it]
	show footnote: it => text(fill: priv-primary.get())[#it]
	body
}
...
#let my-block(
	...
	stack(
		...
		block(
			...
			#text(size: 16pt, fill: colors.text_title, weight: "bold")[#title]
		],
		block(
			width: 100%,
			height: auto,
...
#let slide(
	...
	slide-ctr.step()
	let inner = inner-slide(title: title, ctr: slide-ctr)[#v(1em) #body]
	later-ctr.update(0)
	pos-ctr.update(0)
	sub-ctr.update(1)
	inner
