    ...
    grid(
      ...
      block(
        ...
        components.left-and-right(
          text(fill: self.colors.primary, weight: "bold", size: 1.4em, utils.call-or-display(self, self.store.header)),
          text(fill: self.colors.primary.lighten(20%), utils.call-or-display(self, self.store.header-right)),
        ),
      ),
    )
  }
  let footer(self) = {
    ...
    set text(size: .7em)
    {
      let cell(..args, it) = components.cell(
        ..args,
        inset: 1mm,
        std.align(horizon, text(fill: rgb("000"), it)),
      )
      show: block.with(width: 100%, height: auto)
      grid(
        columns: self.store.footer-columns,
        rows: 1.5em,
        cell(fill: self.colors.primary.lighten(30%), utils.call-or-display(self, self.store.footer-a)),
        cell(fill: self.colors.secondary, utils.call-or-display(self, self.store.footer-b)),
        cell(fill: self.colors.tertiary.lighten(30%), utils.call-or-display(self, self.store.footer-c)),
      )
    }
  }
...
#let title-slide(
  ...
  self = utils.merge-dicts(
    ...
    config-page(margin: (x: 2em, top:0em, bottom:0em))
  )
    ...
    std.align(
      center + horizon,
      {
        ...
        block(
          ...
          breakable: false,
          {
            text(size: 2.4em, fill: self.colors.primary, strong(info.title))
