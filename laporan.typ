...
To motivate our #sans("RC-TCTP") formulations, we shall begin with a novel model whose objective utilizes the Cobb--Douglas production function to estimate project costs without requiring known crash costs. The non--linearity of the Cobb--Douglas utility motivates a genetic algorithm (GA) approach that proved to be inefficient. To solve this, we simplify the model in two ways. First, we formulate a mode--based variant employing mixed--integer linear programming (MILP) that utilized branch and bound methods. Second, we also created a time--based variant through constrained programming (CP) that used hybrid boolean satisfiability (SAT) techniques. Though the two alternatives might not seem very motivated as of now, we will later derive them as very natural reductions of the original Cobb--Douglas formulation. 
